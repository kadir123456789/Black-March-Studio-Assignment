...
public class SceneLikeCamera : MonoBehaviour
{
    ...
    [SerializeField] private float speedUpSpeed = 20.0f;
    ...
    private void LateUpdate()
    {
        ...
        if (Input.GetKey(speedUpKey))
            moveSpeed = originalMoveSpeed + speedUpSpeed;
        if(!Input.GetKey(speedUpKey))
        ...
        {
            move += Vector3.up * mouseMoveY * -moveSpeed * Time.deltaTime;
            move += Vector3.right * mouseMoveX * -moveSpeed * Time.deltaTime;
        }

        //Rotate the camera when anchored
        if (Input.GetKey(anchoredRotateKey)) 
        {
            transform.RotateAround(transform.position, transform.right, mouseMoveY * -rotationSpeed * Time.deltaTime);
            transform.RotateAround(transform.position, Vector3.up, mouseMoveX * rotationSpeed * Time.deltaTime);
        }
